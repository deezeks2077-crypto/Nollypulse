...
const Navbar = () => {
  ...
  const handleClose = () => {
                ...
                <div className='ml-auto flex items-center gap-10 max-md:hidden'>
                    {
                        navLinks.map((item, index) => (
                            <Link key={index} href={item.url} className='hover:text-indigo-600 transition-all duration-200'>{item.label}</Link>
                        ))
                    }
                </div>
                { 
    ...
    </div> : <Link href={"/auth/signin"} className='ml-10 border px-4 py-1
                 flex items-center gap-1 hover:text-indigo-600  transition-all duration-200 
                 max-md:ml-auto z-50'><FiUser /> <p className='max-md:hidden'>Sign In</p></Link>
}
